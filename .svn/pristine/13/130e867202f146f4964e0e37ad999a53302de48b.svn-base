...
public class ExcelWaterRemarkUtils {
	...
	public static void main(String[] args) throws Exception {  
		FileInputStream excelFileInputStream = new FileInputStream("H:/testsssss/11.xlsx");
		Workbook wb = new XSSFWorkbook(excelFileInputStream);
		int sheetNum=wb.getNumberOfSheets();
		for (int i = 0; i < sheetNum; i++) {
			putWaterRemarkToExcel(wb, wb.getSheetAt(i), createWaterMark("网段"), 0, 10, 9, 52, 3, 50, 0, 0);
		}
		...
		File file1 = new File("H:/testsssss/11.xlsx");// Excel文件生成后存储的位置。
     ...
     * @throws IOException 
     */
    private static void putWaterRemarkToExcel(Workbook wb, Sheet sheet, BufferedImage bufferImg, int startXCol, int startYRow,  
